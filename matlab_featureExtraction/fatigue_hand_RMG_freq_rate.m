...
clear all
Casenum='1';    
% RoutineNList={'standL_eyeOpen','standL_eyeClose','standR_eyeOpen','standR_eyeClose',...
%     'standboard_eyeOpen','standboard_eyeClose'};
%  RoutineNList={'grip','g','p23','',...
%     };
RoutineNList={'grasp','p2','p23','gripper','fingerInst','GripRing'...
    };

RoutineN=RoutineNList{4};
RNum=1;
saveOPt=1;
% ReloadOpt=1;
...
[br_rmgOpt,pk_rmgOpt,Var_rmgOpt,VarFeature_rmgOpt,h_rmgOpt] = brEstAvg(Ch_data_filt(:,RMG_ch_opt),fsDS,opts3,Chan_Name(RMG_ch_opt),Motion);
[br_emgOpt,pk_emgOpt,Var_emgOpt,VarFeature_emgOpt,h_emgOpt] = brEstAvg(Ch_data_filt(:,EMG_ch_opt),fsDS,opts3,Chan_Name(EMG_ch_opt),Motion);
...
[max_coef_seg(i),max_r_seg(i),~,~,dist(i)]=max_cor(Ch_data_filt,[StartEndT(1,i),StartEndT(2,i)],fsDS,RMG_ch_opt,EMG_ch_opt,0,Chan_Name);
...
% figName = [saveFigFolder,'rmg','_ch_',num2str(RMG_ch_opt)];
% print(h_rmgOpt,[figName,'.tiff'],'-dtiff','-r300');
% savefig(h_rmgOpt,[figName,'.fig']);
...
save([saveMatFolder,'VarFeat','_ch',num2str(RMG_ch_opt),'_',num2str(EMG_ch_opt),'.mat'],...
    'VarFeature_rmgOpt','Var_rmgOpt','VarFeature_emgOpt','Var_emgOpt','RMG_ch_opt','EMG_ch_opt',...

...
% lag
close all
load(['D:\legRMG\data\','ML_comp_feat\feat_v1_all\feat_v3_all.mat']);
for i=16
    
main(Exp_Name_list{i},File_Name_list{i})

end
function main(ExpDate,fileName)
       % Casenum=cN;    
...
% try 
%     load([saveMatFolder,'_Feat_allCh','.mat'],'toff');
% end
...
h_w1=plotWaveform_s(Ch_data_filt(:,ind),Chan_Name(ind),ind,fsDS,[fileName,'self CH']);

h_w2=plotWaveform_s(Ch_data_filt(:,ind2),Chan_Name(ind2),ind2,fsDS,[fileName,'cross CH']);
...
if ReloadOpt==1
 ...
 load([dataPath,'mat','\',fileName,'\VarFeat','.mat'],'RMG_ch_opt','EMG_ch_opt',...
  'sign_opt');
       ...
       load([saveMatFolder,'_Feat_allCh','.mat'],'RMG_ch_opt','EMG_ch_opt',...
    'sign_opt');   
 fprintf('reload opt ch from comp_feat');
    end
% try 
%     load([saveMatFolder,'_Feat_allCh','.mat'],'RMG_ch_opt','EMG_ch_opt',...
%   'sign_opt');
...
figName = [saveFigFolder,'segCor_fit_Fig_final','_ch',num2str(RMG_ch_opt),'_',num2str(EMG_ch_opt)];
print(h_segCor_fit,[figName,'.tiff'],'-dtiff','-r300');
savefig(h_segCor_fit,[figName,'.fig']);

save([saveMatFolder,'_Feat_allCh','.mat'],...
    'featAll_ch','fit_parameter_TimeLag','RMG_ch_opt','EMG_ch_opt','sign_opt');
